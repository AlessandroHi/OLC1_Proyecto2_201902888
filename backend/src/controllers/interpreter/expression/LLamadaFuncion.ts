...
export class LlamadaFuncion extends Expression {
  ...
  public drawAst(): { rama: string; nodo: string } {
    ...
    // generar el nodo
    const nodoPrincipal = `nodoLlamdaFun${id.toString()}`;
    const ramaFuncion = `${nodoPrincipal}[label="Funcion"];\n
    nodoLlamdaFun${nodoPrincipal}[label="${this.id}"];\n
    ${nodoPrincipal} -> nodoLlamdaFun${nodoPrincipal};\n`;
